...
/**
 * Compound operator: Wilson-Balding leaf SPR move + per-site joint Gibbs
 * resample of all leaf genotypes at K randomly chosen sites.
 *
 * <p>Picks a random leaf, detaches its parent, and re-attaches above a random
 * destination edge with a new uniformly-drawn height. Because the topology
 * change is large (much larger than a narrow exchange), the genotype
 * configurations that fit the new tree differ at many sites; per-site joint
 * Gibbs proposals retune all leaves' genotypes at the chosen sites so the new
 * tree is judged on its integrated-out marginal likelihood at those sites
 * rather than against stale genotypes.</p>
 *
 * <p>Hastings ratio with K = all sites collapses to:</p>
 *
 * <pre>
 *   alpha = P(T')/P(T) * (newRange/oldRange)
 *           * prod_s [P(reads_s | T') / P(reads_s | T)]
 * </pre>
 *
 * <p>i.e. the marginal-likelihood ratio over resampled sites, with genotypes
 * integrated out. See {@link ExchangeSiteGibbsOperator} for the narrow
 * exchange variant and detailed math.</p>
 */
@Description("Wilson-Balding leaf SPR + per-site joint Gibbs resample at K sites. " +
        "Integrates leaf genotypes out of the tree-move acceptance ratio at the chosen sites.")
public class WilsonBaldingSiteGibbsOperator extends TreeOperator {
    ...
    private double proposalImpl() {
        ...
        // Wilson-Balding: pick random leaf i, destination edge above j.
        final int leafNodeCount = tree.getLeafNodeCount();
        final Node i = tree.getNode(Randomizer.nextInt(leafNodeCount));
        final Node p = i.getParent();
        if (p == null || p.isRoot()) return Double.NEGATIVE_INFINITY;
        ...
        if (j.isRoot()) return Double.NEGATIVE_INFINITY;
        assert jP != null;
        ...
        final double newMinAge = Math.max(i.getHeight(), j.getHeight());
        final double newRange = jP.getHeight() - newMinAge;
        final double newAge = newMinAge + Randomizer.nextDouble() * newRange;
        final double oldMinAge = Math.max(i.getHeight(), CiP.getHeight());
        final double oldRange = PiP.getHeight() - oldMinAge;
        if (oldRange == 0 || newRange == 0) {
            // zero-length branches: would catapult i anywhere; rule out for symmetry.
            return Double.NEGATIVE_INFINITY;
        }
        final double logHR_wb = Math.log(newRange / Math.abs(oldRange));

        // Pick K eligible sites and snapshot under current tree T.
        ...
        // Wilson-Balding move T -> T'.
        // Disconnect p from PiP (its old parent) and CiP (other child takes p's spot),
        // then re-attach p above edge to j: p becomes parent of j and i.
        replace(PiP, p, CiP);
        replace(p, CiP, j);
        replace(jP, j, p);
        p.setHeight(newAge);
